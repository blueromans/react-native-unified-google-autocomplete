...
>((props, ref) => {
   ...
   */
  const _enableRowLoader = useCallback((rowData: ResultType) => {
    ...
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      if (row && (row.place_id === rowData.place_id || (row.isCurrentLocation === true && rowData.isCurrentLocation === true))) {
   ...
   */
  const _renderDescription = useCallback((rowData: ResultType): string => {
    ...
    (text: string) => {
      ...
      if (
        ...
        let options: RequestInit = {
          ...
          signal,
          credentials: requestShouldUseWithCredentials(url) ? 'include' : 'omit',
        };
        ...
        fetch(requestUrl, options)
          ...
          .then((responseJSON) => {
            if (typeof responseJSON.predictions !== 'undefined') {
              ...
              console.warn('google places autocomplete: Unexpected response structure', responseJSON);
              if (props.onFail) {
   ...
   */
  const _handleChangeText = useCallback((text: string) => {
    _onChangeText(text);
  ...
  const getCurrentLocation = useCallback(() => {
    ...
    _disableRowLoaders,
    props.currentLocationLabel,
    props.enableHighAccuracyLocation,
    props.nearbyPlacesAPI,
    props.onPress,
  ]);
   ...
   */
  const _onPress = useCallback((rowData: ResultType) => {
    if (rowData.isPredefinedPlace !== true && props.fetchDetails === true) {
      ...
      if (props.isNewPlacesAPI) {
        ...
        requestUrl =
          ...
          Qs.stringify({
            ...
            language: (props.query as any).language,
      ...
      const predefinedPlace = getPredefinedPlace(rowData, props.predefinedPlaces);
   ...
   */
  const _renderLoader = useCallback((rowData: ResultType) => {
    if (rowData.isLoading === true) {
      return (
        <View
          style={[
   ...
   */
  const _renderRowData = useCallback((rowData: ResultType, index: number) => {
    if (props.renderRow) {
  ...
  }, [_renderDescription, props.numberOfLines, props.renderRow, props.styles, props.suppressDefaultStyles]);

  /**
   * Renders a single row in the FlatList
   */
  const _renderRow = useCallback((item: ListRenderItemInfo<ResultType>) => {
    const rowData = item.item;
    return (
      <TouchableHighlight
        style={{
          minWidth: '100%',
          backgroundColor: props.listHoverColor || '#c8c7cc',
        }}
  ...
  }, [_onPress, _renderLoader, _renderRowData, props.listHoverColor, props.listUnderlayColor, props.styles, props.suppressDefaultStyles]);
  ...
  const _getListView = useCallback(() => {
    ...
    return _getFlatList();
  }, [_getFlatList, dataSource.length, listLoaderDisplayed, props.listEmptyComponent, props.listLoaderComponent]);
  ...
  const _renderPoweredLogo = useCallback(() => {
    ...
    return (
      ...
      >
        <Image
          resizeMode="contain"
          source={{ uri: 'https://maps.gstatic.com/mapfiles/api-3/images/powered-by-google-on-white3.png' }}
          style={[
  ...
  }, [props.enablePoweredByContainer, props.styles, props.suppressDefaultStyles]);
  ...
  /**
   * Renders the component to be displayed in between the TextInput and the list view
   */
  ...
  }, [_handleChangeText, _onBlur, _onFocus, props.placeholder, props.styles, props.suppressDefaultStyles, props.textInputProps, stateText]);
  ...
  }, [getCurrentLocation, props.currentLocation, props.currentLocationLabel, props.predefinedPlaces, props.predefinedPlacesAlwaysVisible, stateText]);
